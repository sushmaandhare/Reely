...
class TeamMemberListVC: UIViewController, UITableViewDataSource, UITableViewDelegate {
            ...
            case .success( let value):
                ...
                if(code == "200"){
                    if let myCountry = dic["msg"] as? [[String:Any]]{
                        ...
                        self.TeamMemberArray.removeAll()
    ...
    @objc func onTapFollow(_ sender:UIButton){
        let obj = self.TeamMemberArray[sender.tag]
        onTapFollow(status: obj.follow_Status ?? "", followedFbId: obj.fb_id ?? "")
    }
    ...
    private func onTapFollow(status: String , followedFbId: String){
        ...
        let parameter:[String:Any]?  = ["fb_id":UserDefaults.standard.string(forKey: "uid")!,
                                        "followed_fb_id": followedFbId,
                                        "status": (status == "Follow") ? "1" : "0",
